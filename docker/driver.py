...
def process_cmd(yaml_file, local=False):
    ...
    with open(f"{job_name}_logging.log", 'a') as fout:
        if local:
            subprocess.Popen(f"export PYTHONPATH='/home/ubuntu/FedScale' && {ps_cmd}", shell=True, stdout=fout, stderr=fout)
        else:
        ...
        for cuda_id in range(len(gpu)):
            for _ in range(gpu[cuda_id]):
                ...
                with open(f"{job_name}_logging.log", 'a') as fout:
                    time.sleep(2)
                    if local:
                        subprocess.Popen(f"export PYTHONPATH='/home/ubuntu/FedScale' && {worker_cmd}",
                                         shell=True, stdout=fout, stderr=fout)
                    else:

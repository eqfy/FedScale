...
class GlueflClientManager(clientManager):
    def __init__(self, mode, args, sample_seed=233):
        ...
        numOfClientsOvercommit = round(args.num_participants * (args.overcommitment - 1.0))
        if args.overcommit_weight >= 0:
            self.change_num = round(args.overcommit_weight * numOfClientsOvercommit + args.sticky_group_change_num)
        else:
    ...
    def select_participants_sticky(self, cur_time):
        ...
        logging.info(f"Sticky sampling num {self.sample_num} K (sticky group size) {self.sticky_group_size} Change {self.change_num}")
        ...
        if len(self.sticky_group) == 0:
            ...
            client_len = round(min(self.sticky_group_size, len(clients_online) -1) * self.overcommitment)
            temp_group = sorted(clients_online[:client_len], key=lambda c: min(self.getBwInfo(c)))
            ...
            selected_sticky_clients = online_sticky_group[:(self.sample_num - self.change_num)]
    ...
    def presample_sticky_b(self, round: int, cur_time: float, completed_clients = []):
        ...
        if (round == 1):
            for _ in range(self.max_prefetch_round):
                ...
                self.sampled_changed_clients.append(changed_client)
                # Uniform
                new_changed_clients = self.rng.sample(changed_client, min(self.change_num, len(changed_client)))
    ...
    def presample_sticky_c(self, round: int, cur_time: float, completed_clients = []):
        ...
        if (round == 1):
            for _ in range(self.max_prefetch_round):
                ...
                changed_client_probability = softmax([min(self.getBwInfo(c)) for c in changed_client])
                new_changed_clients = np.random.choice(changed_client, min(self.change_num, size=len(changed_client)), replace=False,
                p=changed_client_probability).tolist()
                self.update_sticky_group(new_changed_clients)

...
class GlueflClient(TorchClient):
    ...
    def train(
        ...
        for idx, param in enumerate(model.state_dict().values()):
            ...
            gradient_tmp = (
                (last_model_copy[idx] - param.data)
            ...
            if not (("num_batches_tracked" in keys[idx]) or ("running" in keys[idx])):
                ...
                # ===== apply compression =====
                if (
                    fl_method in ["STC", "STCPrefetch"]
                    or epochNo % regenerate_epoch == 1
                ):
                    # local masking
                    gradient_tmp, ctx_tmp = compressor.compress(gradient_tmp)
            ...
            compensation_model[idx] = (
                gradient_original.type(torch.FloatTensor)
                - gradient_tmp.type(torch.FloatTensor)
            ).type(torch.FloatTensor) * agg_weight
            # if agg_weight > 100.0:
